...
router.post("/", [
    ...
    con.query(`SELECT * FROM PROJECT WHERE project_id='${projectId}' AND status='SUBMITTED'`, (err, result) => {
        if (err) return res.status(501).json({ error: err.sqlMessage });
        if (result.length === 0) return res.status(400).json({ error: "Project not found" });
